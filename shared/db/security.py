"""Symmetric encryption for secrets stored in `*.secrets_encrypted` columns.

Used by repositories under `modules/publishing/infrastructure/` and
`modules/ingestion/infrastructure/` so OAuth tokens and webhook secrets
