...
def test_inspect_ingestion_source_returns_record_when_found() -> None:
    record = _make_record("agency-1", "src-1", "ckp.ie")
    sources = _SourcesRepo({"agency-1": (record,)})
    agencies = _AgenciesRepo({"agency-1": _agency("agency-1")})

    result = InspectIngestionSourceUseCase().execute(
...
def test_inspect_ingestion_source_raises_when_agency_missing() -> None:
    with pytest.raises(ResourceNotFoundError) as exc_info:
        InspectIngestionSourceUseCase().execute(
            uow=_uow(sources=_SourcesRepo({}), agencies=_AgenciesRepo({})),
            agency_id="ghost",
            ingestion_source_id="src-x",
        )
    assert exc_info.value.code == "ADMIN_AGENCY_NOT_FOUND"


def test_inspect_ingestion_source_raises_when_id_unknown() -> None:
    sources = _SourcesRepo({"agency-1": ()})
    agencies = _AgenciesRepo({"agency-1": _agency("agency-1")})
...
def _make_record(agency_id: str, ingestion_source_id: str, external_id: str) -> IngestionSourceWithAgency:
