"""FastAPI router for the agency outro-video upload endpoints (feature 33).
...
The multipart parsing reuses the stdlib ``email`` strategy from
``brand_logo_router.py`` and ``music_upload_router.py`` so we don't add
the ``python-multipart`` dependency. Authorization goes through
...
from modules.configuration.application.use_cases.delete_outro_video import (
    DeleteOutroVideoInput,
    DeleteOutroVideoUseCase,
)
...
def create_outro_router(
    *,
    ...
    async def upload_admin_agency_outro(
        agency_id: str,
        request: Request,
    ) -> JSONResponse:
        ...
        if not content_type_header.lower().startswith("multipart/form-data"):
            return json_error(
                415,
                "Outro upload requires multipart/form-data.",
                code="OUTRO_UPLOAD_UNSUPPORTED_TYPE",
        ...
        try:
            ...
            return json_error(
                ...
                code="OUTRO_UPLOAD_MALFORMED",
            )
        if field is None:
            return json_error(
                422,
                "The 'file' multipart field is required.",
                code="OUTRO_UPLOAD_MISSING_FIELD",
            )
        ...
        try:
            ...
            logger.exception(
                "Outro upload failed for agency=%s", agency_id,
            )
            return json_error(
                500,
                str(error),
                code=getattr(error, "code", "OUTRO_UPLOAD_FAILED"),
                hint=error.hint,
                details={"context": error.context} if error.context else None,
            )
    ...
    @router.get(
        "/agencies/{agency_id}/outro/file",
        summary="Stream the agency's previously uploaded outro video",
        description=(
            "Serves the bytes of the outro registered for the agency. "
            "Returns 404 if no outro is configured or the on-disk blob has "
            "been removed."
        ),
    )
    async def stream_admin_agency_outro(
        agency_id: str,
        request: Request,
    ) -> Response:
        authorization_error = authorize_admin_request(request, admin_access_policy)
        ...
        if asset is None or asset.source != "uploaded" or not asset.object_key:
            return json_error(
                404,
                "Outro video not configured.",
                code="OUTRO_FILE_NOT_FOUND",
        ...
        if path is None:
            return json_error(
                404,
                "Outro video binary not found on disk.",
                code="OUTRO_FILE_NOT_FOUND",
                details={"agency_id": agency_id},
            )
    ...
    @router.delete(
        "/agencies/{agency_id}/outro",
        summary="Delete the agency's outro video",
        description=(
            "Resets the outro metadata to ``source='none'`` (object_key "
            "cleared) and removes the on-disk blob. Idempotent — a DELETE "
            "without a prior upload returns the same shape."
        ),
    )
    async def delete_admin_agency_outro(
        agency_id: str,
        request: Request,
    ) -> JSONResponse:
        authorization_error = authorize_admin_request(request, admin_access_policy)
        ...
        try:
            with unit_of_work_factory() as uow:
                asset = delete_use_case.execute(
                    uow=uow,
                    data=DeleteOutroVideoInput(agency_id=agency_id),
                )
        except ResourceNotFoundError as error:
            return json_error(
                404,
...
def _serialize_asset(asset: IntroOutroAsset) -> dict[str, object]:
    ...
    return {
        "outro_object_key": object_key or None,
        "outro_duration_seconds": duration_seconds,
        "outro_source": asset.source,
    }


__all__ = ["create_outro_router"]
