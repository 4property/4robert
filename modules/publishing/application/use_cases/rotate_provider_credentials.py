"""Rotate (replace) the stored credentials of an existing provider connection."""
...
class RotateProviderCredentialsInput:
    agency_id: str
    location_id: str
    access_token: str
...
class RotateProviderCredentialsUseCase:
    """Update tokens / location / status on the agency's existing connection.

    Mirrors `AttachProviderConnectionUseCase` but requires a row to already
    exist for `(agency_id, provider)`. If none exists, it raises
    `GHL_CONNECTION_NOT_FOUND` so the caller can attach instead of rotate.
    """
    ...
    def execute(
        ...
        *,
        ...
        existing = uow.publishing.connections.get_by_agency_and_provider(
            agency_id=agency_id,
