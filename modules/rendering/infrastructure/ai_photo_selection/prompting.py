"""Gemini photo-selection prompt + property-context builder.

Migrated from ``services/ai/photo_selection/prompting.py`` during
sub-feature 18c. Owns text/HTML normalization helpers, slug/int parsers
and the ``build_prompt`` template fed to the Gemini classification
client.
"""
...
def build_property_context(property_item: Property) -> dict[str, Any]:
    ...
    property_accommodation = html_to_text(property_item.property_accommodation)
    ...
    if property_accommodation:
        context_lines.append(f"Property accommodation: {property_accommodation}")
    ...
    return {
        ...
        "property_accommodation": property_accommodation,
...
Caption and highlights source-of-truth rules:
- Generate "caption" and "highlights" PRIMARILY from the "Main features provided by the agent or listing" block above. Reuse its exact wording and terminology whenever it matches what is visible in the image.
- Only fall back to the property accommodation / Description / Excerpt as a SECONDARY source when the main features block does not provide enough information to describe what is clearly visible in the image.
- Never invent details that are not present in either the main features block or the property accommodation / description / excerpt.
- Do not repeat the same information across captions or highlights of different photos of the same property. Each caption and each highlight must contribute a distinct fact; if a feature has already been used on another slide, pick a different feature from the main features block (or, as fallback, from the property accommodation / description) to keep every slide unique.
